...
''',
    ...
    def test_cli_env_overrides_profile_env(self):
        with tempfile.TemporaryDirectory() as td:
            path = self.write_config(
                td,
                '''
...
''',
            ...
            argv, context = templates.apply_template_profile(
                [
                    ...
                    "lite",
                    "--env",
                    "ENABLE_SAGE_UPLOAD=false;EXTRA=yes",
                ],
        ...
        self.assertEqual(
            context.env,
            {"ENABLE_SAGE_UPLOAD": "false", "MINMAX": "true", "EXTRA": "yes"},
        )
    ...
    def test_api_hook_resolves_alias_and_adds_env(self):
