...
VARS=(
  ...
  PYTHONUNBUFFERED
  HF_HOME HF_HUB_ENABLE_HF_TRANSFER
)
...
#------------------------------------------------------------------------
section 0 "Prepare Pod for bootstrap"
#----------------------------------------------
...
#------------------------------------------------------------------------
section 1 "Install Baseline Packages"
...
#----------------------------------------------
section 2 "Wire up .env and helpers"
#----------------------------------------------

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
ENVIRONMENT="${ENVIRONMENT:-$SCRIPT_DIR/.env}"
COMMON="${COMMON:-$SCRIPT_DIR/secourses/lib/runtime_common.sh}"
...
if [[ ! -f "$COMMON" ]]; then
  print_err "[fatal] runtime_common.sh not found at: $COMMON" >&2
  exit 1
fi
# shellcheck source=/dev/null
source "$COMMON"
...
#----------------------------------------------
section 4 "Copy contents of SECourses zip files to /workspace"
#----------------------------------------------
...
#----------------------------------------------
section 5 "Install and launch training GUI (if enabled)"
#----------------------------------------------
...
#----------------------------------------------
section 6 "Install and launch Image Processing GUI (if enabled)"
#----------------------------------------------
