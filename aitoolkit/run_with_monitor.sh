...
# --- args ---
[[ $# -ge 1 ]] || die "usage: $0 <run_name_or_outdir> [optional: training_config.yaml]"
RUN_ARG="$1"; shift || true
CFG_ARG="${1:-}"  # optional override

# --- constants ---
AITK_ROOT="/app/ai-toolkit"
OUT_BASE="${AITK_ROOT}/output"
DATASETS_DIR="${AITK_ROOT}/datasets"
...
hff_get_extract_tar_if_missing_dir() {
  local remote_tar="$1"   # e.g. training/5h1v.tar
  local target_dir="$2"   # e.g. datasets/5h1v
  local tar_name
  tar_name="$(basename "$remote_tar")"

  if [[ -d "${AITK_ROOT}/${target_dir}" ]]; then
    return 0
  fi

  echo "Fetching ${remote_tar}..."
  hff get "$remote_tar"
  [[ -f "${AITK_ROOT}/${tar_name}" ]] || [[ -f "${tar_name}" ]] || die "expected tar file ${tar_name} after hff get"

  # tar file likely lands in cwd; ensure we run from AITK_ROOT
  tar -xf "${tar_name}" -C "${AITK_ROOT}/datasets"
}
...
hff_get_extract_tar_if_missing_dir "training/5h1v.tar" "datasets/5h1v"
hff_get_extract_tar_if_missing_dir "training/5h1v_control_images.tar" "datasets/5h1v_control_images"

# --- training config ---
# You wanted: if not present, pull "training/<RUN_NAME>.yaml" and store it as training_config.yaml
CFG_LOCAL="${OUTDIR}/training_config.yaml"

if [[ -n "$CFG_ARG" ]]; then
  # allow explicit config path (relative or absolute)
  if [[ "$CFG_ARG" = /* ]]; then
    CFG_SRC="$CFG_ARG"
  else
    CFG_SRC="${PWD}/${CFG_ARG}"
  fi
  [[ -f "$CFG_SRC" ]] || die "config not found: $CFG_SRC"
  cp -f "$CFG_SRC" "$CFG_LOCAL"
elif [[ ! -f "$CFG_LOCAL" ]]; then
  REMOTE_CFG="training/${RUN_NAME}.yaml"
  echo "Fetching config ${REMOTE_CFG}..."
  hff get "$REMOTE_CFG"
  [[ -f "${AITK_ROOT}/${RUN_NAME}.yaml" || -f "${RUN_NAME}.yaml" ]] || die "expected ${RUN_NAME}.yaml after hff get"
...
# export TRAIN_PID_PATH="${OUTDIR}/trainpid.txt"
...
# --- start training ---
echo "Starting training with config: ${CFG_LOCAL}"
python run.py "$CFG_LOCAL" > "${OUTDIR}/train.log" 2>&1 &
TRAIN_PID=$!
